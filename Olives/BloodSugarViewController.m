...
//
#define APIURL_BLOODSUGAR @"http://olive.azurewebsites.net/api/bloodsugar/filter"
...
@property (strong,nonatomic) NSArray *bloodSugarArray;
...
-(void)loadBloodSugarDataFromAPIFromDate:(double)minTime toDate: (double)maxTime{

    // create url
    NSURL *url = [NSURL URLWithString:APIURL_BLOODSUGAR];
                                                       ...
                                                       completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
                                      {
                                          ...
                                          if((long)[httpResponse statusCode] == 200  && error ==nil)
                                          {
                                              ...
                                              if (self.responseJSONData != nil) {
                                                  self.bloodSugarArray = [self.responseJSONData objectForKey:@"BloodSugars"];
...
-(void)configModeForDisplay:(NSInteger)totalDays{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self loadBloodSugarDataFromAPIFromDate:fromdate*1000 toDate:todate*1000];
...
-(void)displayDataToChartViewWithTotalDays:(NSInteger) total fromStartDate:(NSDate*)startDate{
    ...
    for (int index = 0; index < timeArray.count; index ++) {
        ...
        for (int index = 0; index<self.bloodSugarArray.count; index++) {
            NSDictionary *currentDic = self.bloodSugarArray[index];
            //get value
            NSString *rateValue = [currentDic objectForKey:@"Value"];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return self.bloodSugarArray.count;
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    //    NSDictionary *addDic = [self.addictionArray objectAtIndex:indexPath.row];
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"bloodSugarCell" forIndexPath:indexPath];
    NSDictionary *currentDic = [self.bloodSugarArray objectAtIndex:indexPath.row];

    // Configure the cell...
    NSString *timeString = [currentDic objectForKey:@"Time"];
    NSString *rate = [currentDic objectForKey:@"Value"];
    NSDateFormatter * dateFormatterToLocal = [[NSDateFormatter alloc] init];
    [dateFormatterToLocal setTimeZone:[NSTimeZone systemTimeZone]];
    [dateFormatterToLocal setDateFormat:@"MM:dd:yyyy"];
